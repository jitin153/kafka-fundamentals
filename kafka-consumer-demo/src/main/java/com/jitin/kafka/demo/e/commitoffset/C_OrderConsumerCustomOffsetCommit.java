...
public class C_OrderConsumerCustomOffsetCommit {
		...
		try {
			while(true) {
				...
				for (ConsumerRecord<String, Integer> order : orders) {
					...
					// Committing offset after every 10 records processed.
					if(count%10==0) {
						orderConsumer.commitAsync(Collections.singletonMap(new TopicPartition(order.topic(), order.partition()), new OffsetAndMetadata(order.offset()+1)),new OffsetCommitCallback() {
				 ...
				 * Why it doesn't support retCommitries?
